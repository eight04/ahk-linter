...
const {tokensToEscapedString, parseEscapedString} = require("./command");
...
function parseHotkeyStatement(state) {
  ...
  state.value = {
    ...
    modifier: tokensToEscapedString(state, modifiers),
    prefixKey,
    key,
    execute: command,
...
function parseHotstringStatement(state) {
  ...
  state.value = {
    ...
    options,
    hotstring: tokensToEscapedString(state, hotstring),
    execute: command,
    replacement,
    start: startToken.start,
    end: state.token(-1).end,
