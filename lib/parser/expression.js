...
function isForcedExpression(state) {
  const token = state.token();
  const nextToken = state.token(1);
  return token && nextToken && token.value === "%" && nextToken.start !== token.end;
}

function parseForcedExpression(state, stopAt = null) {
  const firstToken = state.token();
  state.i++;
  parseExpression(state, stopAt && OP2_PRECEDENCE.hasOwnProperty(stopAt) ?
    OP2_PRECEDENCE[stopAt] : undefined);
  if (!state.value) {
    throw new ParseError(state);
  }
  state.value = {
    type: "ForcedExpression",
    argument: state.value,
