...
function parseFlowStatement(state) {
  ...
  const parsers = {
    ...
    ifmsgbox: parseIfMsgBoxStatement
  };
...
function parseIfBody(state) {
  ...
  state.value = [consequent, alternate];
}

function parseIfMsgBoxStatement(state) {
  const startToken = state.token();
  state.i++;
  ...
  const buttonName = state.value;
  if (!state.eof() && state.token().value === ",") {
    state.i++;
  }
  parseIfBody(state);
  const [consequent, alternate] = state.value;
  state.value = {
    type: "IfMsgBoxStatement",
    buttonName,
    consequent,
    alternate,
    start: startToken.start,
